...
async function seed() {
  console.log('Seeding all accounts and student data...');
  ...
  // All accounts from CSV
  console.log(`Total accounts in CSV: ${accounts.length}`);

  await withTransaction(async (client) => {
    // Create ALL accounts from CSV that don't exist
    for (const a of accounts) {
      ...
      // Skip super admin from CSV (account 1) since we already have super admin (19)
      if (a.role === 'SUPER_ADMIN') {
        console.log(`Skipping SUPER_ADMIN account ${a.id} (${a.email}) - already have super admin`);
        continue;
      }
      ...
      await client.query(
        ...
        [a.id, a.email, a.full_name || 'User', a.role, passwordHash, a.home_department_id || null, 'ACTIVE', 19, a.created_at, a.updated_at, a.last_login_at || null]
      );
      console.log(`Created account ${a.id}: ${a.email} (${a.role})`);
    }
    ...
    for (const scr of studentCourseRegistrations) {
      if (scr.term_id == 1) {
        ...
        // Fix registered_by to use an existing account (use super admin 19)
        const registeredBy = scr.registered_by === '4' ? '19' : scr.registered_by;
        await client.query(
          ...
          [scr.id, scr.student_id, scr.course_id, scr.term_id, scr.state, registeredBy, scr.registered_at, scr.updated_at]
        );
        regCount++;
      }
    }
    ...
    const groupCounts = { '1': 8, '2': 8, '3': 8, '4': 8 };
    for (const sg of studentGroups) {
      if (sg.term_id == 1) {
        ...
        await client.query(
          ...
          [sg.id, sg.term_id, sg.department_id || null, sg.name, groupCounts[sg.id] || 8]
        );
      }
    }
  ...
  console.log('Complete seeding done!');
  process.exit(0);
}
